...
public interface IGraphicsSettings : IInteractiveHud
{
    ...
    float AutoExposureKey { get; set; }      // target middle-grey (~0.18)
    float AutoExposureMin { get; set; }      // lowest exposure (bright-scene floor)
    float AutoExposureMax { get; set; }      // highest exposure (dark-scene cap; low keeps night dark)
    float AutoExposureSpeed { get; set; }    // adaptation rate (higher = snappier)
    ...
    float SsaoIntensity { get; set; }   // AO strength multiplier

    bool UseSsr { get; set; }           // screen-space reflections (water)
    bool UseContactShadows { get; set; }// screen-space contact shadows
